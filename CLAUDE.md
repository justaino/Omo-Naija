...
- **Recently added:** word **meanings** surface from the `hint` field (won cards on the turn summary; expanded piles on the scoreboard show "Word — meaning"); a **Back** button on Setup (→ Home); a **full-bleed** card layout on phones (≤480px). Also: on a **timeout** the last card on screen is captured as skipped on the turn summary (no skip charged); **selectable themes** — a Theme picker in Settings offers **Classic** (keeps light/dark) plus two committed "skins", **Owambe Gold** and **Highlife** (**Highlife is the default** for anyone who hasn't chosen), via a `data-skin` attribute + `css/skins.css` (self-hosted Playfair Display / Alfa Slab One fonts); a redundant-**eyebrow** cleanup (Setup/Settings/How-to) + eyebrow→title spacing; and **bigger Scoreboard piles** (Live Play card size was tried larger, then reverted to the original at the user's request).
- **Note on the golden rule:** the Owambe/Highlife skins are opt-in re-skins the user explicitly requested; Classic remains the untouched source-of-truth look. The bigger Scoreboard piles were a deliberate, user-approved change.
