...
export function render(el, ctx) {
  ...
  el.innerHTML = `
    <div class="card">
      ...
      <div>
        <div class="screen__eyebrow" style="margin-bottom:10px;">Mode</div>
        <div class="mode-grid">
          <button class="mode-card mode-card--green${s.mode === 'green' ? ' active' : ''}" data-mode="green"><span>Green (3 words + gestures)</span><span>✓</span></button>
          <button class="mode-card mode-card--charcoal${s.mode === 'grey' ? ' active' : ''}" data-mode="grey"><span>Grey (describe freely)</span><span>●</span></button>
          <button class="mode-card mode-card--mixed${s.mode === 'mixed' ? ' active' : ''}" data-mode="mixed"><span>Mixed</span><span>✦</span></button>
        </div>
      </div>
  ...
  el.querySelectorAll('[data-mode]').forEach((b) =>
    b.addEventListener('click', () => { draft.settings.mode = b.dataset.mode; rerender(); }));
