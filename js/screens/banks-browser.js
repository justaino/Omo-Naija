...
function cardHtml(c) {
  const meta = [c.category, c.era, c.difficulty, (c.modes || []).join('/')].filter(Boolean).join(' · ');
  const search = `${c.text || ''} ${c.hint || ''}`.toLowerCase();
  return `
    <div class="bank-card" data-card data-search="${esc(search)}">
      <div class="bank-card__main">
        <span class="bank-card__word">${esc(c.text || '')}</span>
        ${c.hint ? `<span class="bank-card__hint">${esc(c.hint)}</span>` : ''}
      </div>
